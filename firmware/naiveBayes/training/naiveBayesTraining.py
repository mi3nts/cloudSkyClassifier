...
# Description:
# The Application is designed to train a Random Forest Classifier to Recognize Sky/Cloud Pixels. Pixel value sky will be
# given for a Cloud Pixel While a Pixel Value 0 will be give for a Sky Pixels. The Trained Model named
...
if __name__ == '__main__':
    ...
    print('-----------------------------------------------------')
    print("-Cloud Sky Classifier - Training - Naive Bayes Model-")
    print('-----------------------------------------------------')
    ...
    print('-----------------------------------------------------')
    Naive_Based_Classifier = GaussianNB()
    Naive_Based_Classifier.fit(All_Features_Training, np.ravel(All_Targets_Training, order='C'))


    # print("Training FOrest")
    # Random_Forest_Model = RandomForestRegressor(n_estimators=50, random_state=42,verbose=True)
    # # Train the model on training data
    # Random_Forest_Model.fit(All_Features_Training, np.ravel(All_Targets_Training, order='C'))

    Model_Save_File_Name = dataSetPath + 'naiveBayesModel.sav'
    pickle.dump(Naive_Based_Classifier, open(Model_Save_File_Name, 'wb'))
    ...
    Confusion_Matrix_Training_NB = confusion_matrix(All_Targets_Training,trainingPredictionBinary)

    print("Confusion Matrix for Naive Bayes Classifier - Training")
    print(Confusion_Matrix_Training_NB)
    print('-----------------------------------------------------')


    TrainingAccuracy  =round((Confusion_Matrix_Training_NB[0][0]+ Confusion_Matrix_Training_NB[1][1]) \
                               /(Confusion_Matrix_Training_NB[0][0]+ Confusion_Matrix_Training_NB[1][1] + \
                             Confusion_Matrix_Training_NB[0][1]+ Confusion_Matrix_Training_NB[1][0]),4)

    print("Training Accuracy for Naive Bayes Classifier: "+str(round(TrainingAccuracy*100,2))+"%" )
    print("-----------------------------------------------------")
